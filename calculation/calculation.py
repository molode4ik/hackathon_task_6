...
HOME_PERCENTS_LIST = [[0, -6, -12, -17, -22, -24], [6, 0, -7, -12, -17, -19], [14, 7, 0, -6, -11, -13],
                      [21, 14, 6, 0, -6, -8], [28, 21, 13, 6, 0, -3], [31, 24, 16, 9, 3, 0]]
...
class Analog:
    ...
    def search_floor_percent(self, floor: int, total_floor: int):
        gap = self.search_gap(search_floor_lvl(floor, total_floor), FLOORS_LIST)
        analog_gap = self.search_gap(search_floor_lvl(self.floor, self.floor_total), FLOORS_LIST)
        return FLOOR_PERCENTS_LIST[analog_gap][gap]

    def searh_home_square_percent(self, home_area: float):
    ...
    def search_kitchen_square_percent(self, kitchen_area: float):
        if self.kitchen_area > 15:
            self.kitchen_area =14
    ...
    def search_balcony_percent(self, balcony: str):
        gap = self.search_gap(search_balcony(balcony), BALCONY_LIST)
        analog_gap = self.search_gap(search_balcony(self.balcony), BALCONY_LIST)
    ...
    def search_decoration_state_percent(self, repairs: str):
        gap = self.search_gap(search_repair(repairs), DECORATION_STATES_LIST)
        analog_gap = self.search_gap(search_repair(self.repairs), DECORATION_STATES_LIST)
